...
import { getVaDataSelector } from "@/states/store"
...
const VaDetail : NextPage = ( props : any ) => {
    ...
    const name = useRecoilValueLoadable(getVaDataSelector);

    const backUrl = '/asset/va'
...
export async function getStaticPaths (context : any) {
    ...
    let { data } = await axios.get(process.env.NEXT_PUBLIC_CDN_URL + 'safe_va/SAFE_VA.json')

    let paths: any[] = [];

    let trendData = data['TRENDS'].contents;
    trendData.forEach((data: { id : any; }) => paths.push(
        { params : { slug : data.id}}
    ))

    let issueData = data['ISSUE'].contents;
    issueData.forEach((data: { id : any; }) => paths.push(
        { params : { slug : data.id}}
    ))
    let etcData = data['ETC'].contents;
    etcData.forEach((data: { id : any; }) => paths.push(
        { params : { slug : data.id}}
    ))
